...
export default function Dashboard() {
  ...
  // FIXED: Supabase schema (NO reporter usage)
  const citizenIssues = safeIssues.filter(
    (i) => i?.created_by === user?.id
  );
      ...
      <div className="space-y-8 animate-fade-in">
        ...
        <div className="space-y-4">
          ...
          <Card className="p-5 space-y-3.5">
            {safeIssues.length === 0 ? (
              <p className="text-xxs text-muted-foreground text-center">No recent activity</p>
            ) : (
              safeIssues.slice(0, 4).map(issue => (
                <div key={issue.id} className="border-b border-border/40 pb-3 last:border-0 last:pb-0">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-0.5">
                    {issue?.location?.split(',')?.[0] || "Unknown"} •{" "}
